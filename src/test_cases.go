...
const test1 string = `
function Class:function_name(parameter)
    -- Comment
    local reassigned_p = parameter
    string_literal = "string"
    integer = 123456 -- Comment
    decimal = 123.456
    --[[
    ...
    string_literal_with_multiline = "--[[test]]"
    concat_string = "string1" .. "string 2"
    hex = 0x1f4
    bin = 0b1010
    scientific = 1.21e10
    scientific_neg = 1.21e-10
    boolean_literal = true
end

`

const test2 string = `function Layer:draw(options)
    options = options or {}
...
-- TODO - What is up with background color and the origin color why are they reddish?
...
function Color:print(places)
    ...
    local color_table = tostring(Utils.table_to_string(self.__color_table, true, places))
...
function Debug.Logger()
    ...
    local function print_queue(top_padding)
        top_padding = top_padding or 30

        translate { 0, -top_padding }
...
-- 5. Create and manipulate objects under the Create Objects heading
-- 6. Assign objects to the layer tree by placing them in a layer's contents table
-- 7. If necessary, manipulate layer order after tree has been constructed
...
bg = Overlay.new(origin, {name = "Background", color=Color.new()})
...
-- 2. Pack Objects into Layer Tree --
...
----- PRINT CONSOLE -----
console_top_padding = 30

root:draw()
origin:reset()
print(root)
print_all(console_top_padding)
